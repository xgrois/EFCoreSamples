...
static async Task QueryDbAsync(RestaurantDbContextFactory factory)
{
    ...
    // From join table, go to left and right tables
    var dishesToIngredients = await db.DishesToIngredients
        .Include(dti => dti.Dish)
        .Include(dti => dti.Ingredient)
        .ToArrayAsync();

    foreach (var dti in dishesToIngredients)
        Console.WriteLine($"{dti.Dish.Title} has {dti.Ingredient.Name}");

    // Get dishes data, including related data in other tables
    // Dishes -> DishesToIngredients -> Ingredients
    var dishes = await db.Dishes
        .Include(nameof(Dish.DishesToIngredients) + "." + nameof(DishToIngredient.Ingredient))
        .ToArrayAsync();

    foreach (var dish in dishes)
    {
        if (dish.DishesToIngredients.Count == 0)
            Console.WriteLine($"Dish {dish.Title} has no ingredients yet");
        else Console.WriteLine($"Dish {dish.Title} has {String.Join(',', dish.DishesToIngredients.Select(i => i.Ingredient.Name))}");
    }
